...
public class AccountPage extends JPanel implements Page {
	// Setup
	private User user;
	 ...
	 */
	public AccountPage(User user, JFrame frame) {
		//Setup
		this.frame = frame;
		this.user = user;
	...
	private JPanel buildUserInfo(JPanel panel) {
		...
		// User name text
		JLabel usernameDisplay = new JLabel(user.getUsername(), SwingConstants.LEFT);
		...
		// Phone number text
		String phone = ((Integer)user.getPhone()).toString();
		...
		//Email Text
		JLabel emailDisplay = new JLabel(user.getEmail());
	...
	private JPanel buildSettings(JPanel panel) {
		...
		c.insets = new Insets(fieldMargin,0,fieldMargin,fieldMargin);
		c.fill = GridBagConstraints.HORIZONTAL;
		c.ipady = 0;
		c.gridwidth = 1;
		c.weightx = 0.0;
